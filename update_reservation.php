...
// Validate status
if (!in_array($status, ['pending', 'approved', 'rejected', 'returned', 'picked_up', 'active', 'completed', 'cancelled'])) {
    respond(false, 'Invalid status. Must be pending, approved, rejected, returned, picked_up, active, completed, or cancelled');
}
...
try {
    ...
    if ($stmt->execute()) {
        ...
        if ($status === 'approved') {
            ...
            // Restore book stock on return (only if was approved or picked_up)
            if (in_array($currentStatus, ['approved', 'picked_up', 'active'])) {
                $conn->query("UPDATE books SET stock = stock + 1 WHERE id = " . $reservation['book_id']);
            }

        } elseif ($status === 'picked_up') {
            // USER Notification - Book Picked Up
            $userNotification = [
                'title' => "📦 Book Picked Up",
                'body' => "You have collected '$bookTitle'. Due date reminder will be sent."
            ];
            $userData = [
                'type' => 'book_picked_up',
                'reservation_id' => strval($reservationId),
        ...
        // Send notification to USER if status is approved, rejected, returned, or picked_up
        if (in_array($status, ['approved', 'rejected', 'returned', 'picked_up'])) {
