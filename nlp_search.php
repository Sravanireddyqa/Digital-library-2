...
try {
    ...
    // PRIORITY 1: Exact keyword search in title first
    // This ensures "C programming" finds C books, not JavaScript
    $exactBooks = exactKeywordSearch($query, $conn);
    if (!empty($exactBooks)) {
        respond(true, 'Search completed', [
            'books' => $exactBooks,
            'count' => count($exactBooks),
            'nlp' => [
                'intent' => $nlpResult['intent'],
                'category' => $nlpResult['category'],
                'keywords' => $nlpResult['keywords'],
                'understood_as' => 'Finding books matching: ' . $query
            ]
        ]);
        $conn->close();
        exit;
    ...
    // PRIORITY 2: NLP-based category search
...
/**
 * Exact keyword search - searches for exact keywords in title
 * This ensures "C programming" finds C books, not JavaScript
 */
function exactKeywordSearch($query, $conn)
{
    // Clean and prepare the query
    $searchQuery = $conn->real_escape_string($query);

    // Extract main keywords from query (ignore common words)
    $stopWords = ['i', 'want', 'to', 'learn', 'read', 'find', 'show', 'me', 'give', 'books', 'book', 'about', 'on', 'for', 'the', 'a', 'an'];
    ...
    // Build search conditions - prioritize books where title contains the exact keyword
    $conditions = [];
    foreach ($keywords as $kw) {
        $kw = $conn->real_escape_string($kw);
        // Match keyword as a word boundary (not just substring)
        $conditions[] = "(
    ...
    // Require ALL keywords to match (AND condition)
    $sql = "SELECT * FROM books WHERE " . implode(" AND ", $conditions) . " ORDER BY rating DESC LIMIT 20";
    $result = $conn->query($sql);
