...
try {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (strtolower($row['status']) === 'returned') {
            $ratingCheck = $conn->prepare("SELECT id FROM ratings WHERE user_id = ? AND book_id = ?");
            $ratingCheck->bind_param("ii", $userId, $row['book_id']);
            $ratingCheck->execute();
            $isRated = $ratingCheck->get_result()->num_rows > 0;
            $ratingCheck->close();
        }
