...
try {
    ...
    if ($pickupDate < $today) {
        respond(false, 'Pickup date cannot be in the past. Please select today or a future date.');
    }

    // Check if due date is before pickup date
    if ($dueDate < $pickupDate) {
        respond(false, 'Return date cannot be before pickup date.');
    }

    // Check if due date is in the past (only if it's before today)
    if ($dueDate < $today) {
        respond(false, 'Return date cannot be in the past.');
    }
    // ====== END DATE VALIDATION ======
